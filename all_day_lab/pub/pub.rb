...
class Pub
  ...
  def check_age(customer_obj)
    if customer_obj.age > 18
      return true
    else
      return false
    end
  ...
  def too_drunk?(customer_obj)
    if customer_obj.check_drunkeness > 10
      return true
    else
      return false
    end
...
# A Customer should be able to buy a Drink from the Pub, reducing the money in its wallet and increasing the money in the Pub’s till
  ...
  def buy_drink(pub_obj, name_of_drink_str)
    ...
    # is customer of right age?
      if pub_obj.check_age(self) == true &&
         pub_obj.too_drunk?(self) == false
          for drink_obj in pub_obj.drinks
            if drink_obj.name == name_of_drink_str && self.check_wallet > drink_obj.price
              @wallet -= drink_obj.price
              pub_obj.money_in(drink_obj.price)
              pub_obj.inspect_stock[name_of_drink_str] -= 1
              self.drink(drink_obj)
            end
          end
      end
